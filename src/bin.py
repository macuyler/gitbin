...
def get_installed():
    ...
    """
    installed = []
    proc = Popen('ls ~/.macbin', shell=True, stdout=PIPE, stderr=PIPE)
...
def install(args):
    ...
    if len(args) >= 1:
        ...
        if name not in pkgs:
            _, err = Popen(f'cd ~/.macbin/ && git clone {url}',
    ...
    if len(args) == 1:
        ...
        if name in pkgs:
            ...
            with open(f'{Path.home()}/.macbin/.path', 'r') as path:
                ...
                fps = list(filter(lambda x: x.replace(f'{Path.home()}/.macbin/', '')
...
def upgrade(args):
    ...
    if len(args) == 1:
        ...
        if name in pkgs:
            _, err = Popen(f'cd ~/.macbin/{name} && git pull',
